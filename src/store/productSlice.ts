  ...
  async (SubCategoryid: number) => {
    try {
      const data = await axios.get(
        `${url}/GetAllProducts?subCategoryFilter=${SubCategoryid}`
      );
      return data.data;
    } catch (error: any) {
...
export const getAllProductsByCategoriesIds = createAsyncThunk(
  ...
  async (Categoryid: number) => {
    try {
      const data = await axios.get(
        `${url}/GetAllProducts?categoryFilter=${Categoryid}`
      );
      return data.data;
    } catch (error: any) {
      throw error.response.data;
...
export const getAllCartProduct = createAsyncThunk(
  ...
  async (userId: number) => {
    try {
      const data = await axios.get(
        `${url}/GetUserCartDetails?userId=${userId}`
      );
      return data.data;
    } catch (error: any) {
      throw error.response.data;
...
export const getAllWishListProduct = createAsyncThunk(
  ...
  async (userId: number) => {
    try {
      const data = await axios.get(
        `${url}/GetUserWishlistDetails?userId=${userId}`
      );
      return data.data;
    } catch (error: any) {
      throw error.response.data;
...
const ProductsSlice = createSlice({
  ...
  initialState,
  reducers: {
    addItemToWishlist: (state: any, action: any) => {
      const itemExists = state.wishlistData.some(
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(
        getAllProductsByCategoriesIds.fulfilled,
      ...
      .addCase(
        getAllProductsByCategoriesIds.rejected,
        (state: any, action: any) => {
          state.status = "failed";
          state.error = action.error.message;
        }
      )
      .addCase(addToCartProduct.fulfilled, (state: any, action: any) => {
        state.status = "succeeded";
      ...
      .addCase(getAllWishListProduct.pending, (state: any) => {
        state.status = "loading";
        state.error = null;
      })
      .addCase(getAllWishListProduct.fulfilled, (state: any, action: any) => {
        state.status = "succeeded";
        state.wishlistData = action.payload;
      })
      .addCase(getAllWishListProduct.rejected, (state: any, action: any) => {
        state.status = "failed";
        state.error = action.error.message || "Failed to fetch wishlist data";
      })
      .addCase(addToWishListProduct.fulfilled, (state: any, action: any) => {
        state.status = "succeeded";
        state.wishlistData.push(action.payload);
      })
      .addCase(deleteWishListProduct.fulfilled, (state: any, action: any) => {
        state.status = "succeeded";
        state.wishlistData = action.payload;
      })
      .addCase(clearCart, (state: any) => {
        state.cartData = [];
      });
  },
});

export const { addItemToWishlist, clearWishlist } = ProductsSlice.actions;
